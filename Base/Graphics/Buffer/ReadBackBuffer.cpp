...
bool ReadBackBuffer::Initialize(uint64_t size)
{
    if (size == 0)
    {
    ...
    if (rest != 0)
    {
        ELOGA("Error : ReadBackBuffer must be 256 byte alignment!! (size %% 256) = %u", rest);
        return false;
    }
    ...
    //resource
    D3D12_HEAP_TYPE heapType = D3D12_HEAP_TYPE_READBACK;
...
void ReadBackBuffer::SetDebugName(LPCWSTR tag)
{
    if (m_resource.Get())
    {
...
ID3D12Resource* ReadBackBuffer::GetResource() const
{
    return m_resource.Get();
}

D3D12_GPU_VIRTUAL_ADDRESS ReadBackBuffer::GetGpuAddress() const
{
...
uint64_t ReadBackBuffer::GetSize() const
{
    return m_size;
}
